...
async def test_create_user_with_invalid_data(db_session, email_service):
    user_data = {
        "nickname": "",  # Invalid nickname (empty)
...
async def test_get_by_id_user_does_not_exist(db_session):
    # Use a valid UUID string that does not exist in the database.
    non_existent_user_id = "123e4567-e89b-12d3-a456-426614174000"
...
async def test_delete_user_does_not_exist(db_session):
    # Use a valid UUID string that does not correspond to any user.
    non_existent_user_id = "123e4567-e89b-12d3-a456-426614174000"
...
async def test_login_user_incorrect_password(db_session, user):
    logged_in_user = await UserService.login_user(db_session, user.email, "IncorrectPassword!")
...
async def test_verify_email_with_token(db_session, user):
    token = "valid_token_example"  # Simulated valid token for testing.
    user.verification_token = token  # Set the token for the user.
